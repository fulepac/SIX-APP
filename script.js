...
function initSlots() {
    ...
    for(let i=0; i<10; i++) {
        ...
        container.innerHTML += `<div class="obj-slot" id="s-${i}">
            ...
            <input type="number" class="lt" value="${d.la}" style="width:70px">
            <input type="number" class="ln" value="${d.lo}" style="width:70px">
        </div>`;
    }
}
    ...
    if (!r.game.started) {
        ...
        for(let i=0; i<10; i++) {
            ...
            if(s.querySelector(".act").checked) {
                objs.push({ name: s.querySelector(".nm").value, lat: parseFloat(s.querySelector(".lt").value), lon: parseFloat(s.querySelector(".ln").value), owner: "LIBERO", start: null, teamConquering: null });
            }
        }
        r.objectives = objs;
    }
...
function resetBin() { if(confirm("RESET?")) { fetch(URL, {method:"PUT", headers:{"Content-Type":"application/json","X-Master-Key":SECRET_KEY}, body: JSON.stringify({game:{started:false},players:{},objectives:[]})}).then(()=>location.reload()); } }
